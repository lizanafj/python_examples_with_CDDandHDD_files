...
mean1 = "CDD_historical_mean_v1_raster.tif"

mean15 = "CDD_scenario15_mean_v1_raster.tif"

mean20 = "CDD_scenario20_mean_v1_raster.tif"
...
vmin, vmax = 0, 3500
bounds = np.linspace(vmin, vmax, 15)  # 6 intervals = 7 edges
cmap = cm.get_cmap('turbo', 14)      # 6 discrete colors YlOrRd turbo
cmap = ListedColormap(cmap(np.arange(14)))
#cmap.set_under('white')             # white for values < vmin
norm = BoundaryNorm(boundaries=bounds, ncolors=14)
...
for i, (title, raster_path) in enumerate(raster_files):
    ...
    subregions.boundary.plot(ax=ax_map, edgecolor='white', linewidth=0.5, transform=ccrs.PlateCarree())
    ...
    ax_box.set_ylabel("CDD", fontsize=8)
    ax_box.set_ylim(0, 6000)
...
cbar.set_label("Cooling Degree Days (CDD)", fontsize=9)
cbar.ax.tick_params(labelsize=8)

# Set ticks every 500
cbar.set_ticks(np.arange(0, 3501, 500))
...
#plt.savefig("Figure2_CDD_maps_and_boxplots.svg", format='svg', dpi=600, bbox_inches='tight')
plt.savefig("Figure2_CDD_maps_and_boxplots.jpg", format='jpg', dpi=600, bbox_inches='tight')
